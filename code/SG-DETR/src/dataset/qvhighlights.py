...
class QVHighlights(Dataset):  # noqa: WPS230,WPS214
    ...
    def get_query_feat_by_qid(self, qid: int) -> torch.Tensor:
        ...
        with np.load(query_feat_path) as archive:
            query_features = archive["features"].astype(np.float32)
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        """
        # Never mutate the annotation object retained by the Dataset. Mutating
        # it grows every worker's resident heap and breaks fork copy-on-write.
        meta = self.data[index].copy()
