...
from src.model.blocks.encoder import (
    DetectionGuidedSaliencyRefinement,
    DummyEncoder,
    LocalSaliencyHead,
    MomentEncoder,
    SaliencyAmplifier,
...
class MRDETR(nn.Module):
    ...
    def __init__(
        ...
        use_bidirectional_refinement: bool = False,
        ...
        # Bidirectional saliency-detection refinement
        self.use_bidirectional_refinement = use_bidirectional_refinement
        if use_bidirectional_refinement:
            self.det_guided_refinement = DetectionGuidedSaliencyRefinement(model_dim)
    ...
    def forward(  # noqa: WPS234,WPS231
        ...
        **_,
        ...
        video_durations = (
            memory.new_tensor([sample_meta["duration"] for sample_meta in meta])
            if meta is not None
            else None
        )
        ...
        # ---- Bidirectional Saliency-Detection Refinement ----
        if self.use_bidirectional_refinement:
            # use last decoder layer predictions to refine saliency
            with torch.no_grad():
                first_pass_spans = torch.sigmoid(det_output.outputs_coord[-1])  # [B, Q, 2]
                first_pass_logits = det_output.outputs_class[-1]  # [B, Q, 1]

            current_sal = saliency_scores_refined if saliency_scores_refined is not None else saliency_scores
            saliency_scores_bidir = self.det_guided_refinement(
                memory=encoder_output.memory,
                saliency_scores=current_sal,
                pred_spans=first_pass_spans,
                pred_logits=first_pass_logits,
                vid_mask=encoder_output.vid_mask,
            )

            # re-amplify memory with refined saliency
            memory_refined = (
                self.saliency_amplifier(
                    encoder_output.memory,
                    saliency_scores_bidir,
                    encoder_output.vid_pos,
                    encoder_output.vid_mask,
                )
                if self.saliency_amplifier is not None
            ...
            # re-run query selector and decoder with refined memory
            multiscale_refined = self.fpn(memory_refined.transpose(0, 1))
            proposals_output_refined = (
                self.query_selector(multiscale_refined, encoder_output.vid_mask)
                if self.query_selector is not None
                else None
            )

            det_output = self.main_det_head(
                memory_local=memory_refined,
                vid_mask=encoder_output.vid_mask,
                vid_pos=encoder_output.vid_pos,
                matched_gts=aux_head_schema.matched_gts,
                anchors_spans=aux_head_schema.anchors_spans,
                encoder_features=aux_head_schema.selected_features,
                proposals=proposals_output_refined,
                targets=targets,
                video_durations=video_durations,
            )

            # update for output dict
            saliency_scores_refined = saliency_scores_bidir
            if proposals_output_refined is not None:
                proposals_output = proposals_output_refined
        ...
        out = {
            "vid_features": memory,  # [L_vid, B, D] encoder features for boundary loss
        ...
        if det_output.route_logits is not None:
            co_pad_size = det_output.co_info["pad_size"] if det_output.co_info is not None else 0
            dn_pad_size = det_output.dn_info["pad_size"] if det_output.dn_info is not None else 0
            out["route_logits"] = det_output.route_logits[-1, :, co_pad_size + dn_pad_size :, :]
            if det_output.expert_quality_scores is not None:
                out["pred_expert_quality_scores"] = det_output.expert_quality_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.base_quality_scores is not None:
                out["base_quality_scores"] = det_output.base_quality_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.base_class_scores is not None:
                out["base_class_scores"] = det_output.base_class_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.expert_quality_residuals is not None:
                out["expert_quality_residuals"] = det_output.expert_quality_residuals[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.uniform_class_scores is not None:
                out["pred_uniform_logits"] = det_output.uniform_class_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.uniform_quality_scores is not None:
                out["pred_uniform_quality_scores"] = det_output.uniform_quality_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.safety_class_scores is not None:
                out["pred_safety_logits"] = det_output.safety_class_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.safety_quality_scores is not None:
                out["pred_safety_quality_scores"] = det_output.safety_quality_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.route_gate_scores is not None:
                out["route_gate_scores"] = det_output.route_gate_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.base_route_gate_scores is not None:
                out["base_route_gate_scores"] = det_output.base_route_gate_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
            if det_output.route_delta_scores is not None:
                out["route_delta_scores"] = det_output.route_delta_scores[
                    -1, :, co_pad_size + dn_pad_size :, :
                ]
        if det_output.query_length_prior_logits is not None:
            out["query_length_prior_logits"] = det_output.query_length_prior_logits[-1, :, 0, :]
        if det_output.duration_safety_prior_logits is not None:
            out["duration_safety_prior_logits"] = det_output.duration_safety_prior_logits[-1, :, 0, :]
        if det_output.query_prior_safety_logits is not None:
            out["query_prior_safety_logits"] = det_output.query_prior_safety_logits[-1, :, 0, 0]
        if det_output.sample_route_gate_logits is not None:
            out["sample_route_gate_logits"] = det_output.sample_route_gate_logits[-1, :, 0, 0]
