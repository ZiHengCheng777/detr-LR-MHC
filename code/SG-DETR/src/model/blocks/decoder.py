...
class TransformerDecoder(nn.Module):  # noqa: WPS230
    ...
    def __init__(  # noqa: C901, WPS211
        ...
        # Gaussian spatial bias for cross-attention (learnable sharpness)
        # Length-Aware Gaussian: per-class sigma (inspired by LA-DETR, adapted for SG-DETR)
        self.use_gaussian_bias = use_gaussian_bias
        self.length_aware_gaussian = length_aware_gaussian
        self.length_thresholds = length_thresholds
        if use_gaussian_bias:
            if length_aware_gaussian:
                # 3 learnable sigmas, one per length class [short, middle, long]
                # init: short=0.7 (focused), middle=1.0, long=1.3 (wider)
                self.gaussian_sigma = nn.Parameter(torch.tensor([0.7, 1.0, 1.3]))
            else:
    ...
    def forward(  # noqa: WPS210, C901
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            # Length-Aware Gaussian: pick per-query sigma based on current width
            if self.use_gaussian_bias and self.length_aware_gaussian:
                # reference_points: (Q, B, 2) in sigmoid space (center, width)
                widths = reference_points[..., 1]  # (Q, B)
                t1, t2 = self.length_thresholds
                cls = torch.zeros_like(widths, dtype=torch.long)
                cls[widths >= t1] = 1
                cls[widths >= t2] = 2
                # gaussian_sigma: (3,) -> per-query sigma (Q, B)
                per_query_sigma = self.gaussian_sigma[cls]  # (Q, B)
                sigma_arg = per_query_sigma
            else:
                sigma_arg = self.gaussian_sigma if self.use_gaussian_bias else None
            ...
            output = layer(
                ...
                reference_points=reference_points if self.use_gaussian_bias else None,
                gaussian_sigma=sigma_arg,
            )
