...
class DetectionGuidedSaliencyRefinement(nn.Module):
    """Refine saliency scores using decoder moment predictions (bidirectional feedback).

    Takes predicted spans and their confidence from the decoder, builds soft
    Gaussian attention masks, and produces a saliency refinement signal that is
    added back to the original saliency scores.

    Args:
        model_dim: Hidden dimension of the model.
        num_heads: Number of attention heads for the cross-attention refinement.
        temperature: Controls the sharpness of the Gaussian masks.
    """

    def __init__(self, model_dim: int, num_heads: int = 4, temperature: float = 1.0) -> None:
        super().__init__()
        ...
        # initialize gate output near zero so the refinement starts small
    ...
    def forward(
        ...
        """Compute refined saliency scores.

        Args:
            memory: Encoder memory, shape [L_vid, batch_size, model_dim].
            saliency_scores: Current saliency scores, shape [batch_size, L_vid].
            pred_spans: Predicted moment spans (center, width) in [0,1], shape [batch_size, num_queries, 2].
            pred_logits: Predicted classification logits, shape [batch_size, num_queries, 1].
            vid_mask: Video padding mask, shape [batch_size, L_vid].

        Returns:
            Refined saliency scores, shape [batch_size, L_vid].
        """
        L_vid, batch_size, _ = memory.shape

        # build normalized temporal positions [0, 1] for each frame
        # positions: [1, L_vid, 1]
        positions = torch.arange(L_vid, device=memory.device, dtype=memory.dtype) / max(L_vid - 1, 1)
        positions = positions[None, :, None]

        # predicted span centers and widths -> [B, 1, Q] for broadcast with [1, L, 1]
        centers = pred_spans[:, :, 0].unsqueeze(1)  # [B, 1, Q]
        widths = pred_spans[:, :, 1].clamp(min=1e-4).unsqueeze(1)  # [B, 1, Q]

        # confidence weights from logits -> [B, 1, Q]
        confidence = torch.sigmoid(pred_logits.squeeze(-1)).unsqueeze(1)  # [B, 1, Q]

        # Gaussian: exp(-0.5 * ((t - center) / (width * temperature))^2)
        # broadcast: [1, L, 1] - [B, 1, Q] -> [B, L, Q]
        sigma = widths * self.temperature
        diff = positions - centers
        gaussian = torch.exp(-0.5 * (diff / sigma) ** 2)  # [B, L_vid, Q]

        # weight by confidence and aggregate across queries
        weighted_gaussian = gaussian * confidence  # [B, L_vid, Q]
        detection_saliency = weighted_gaussian.sum(dim=-1)  # [B, L_vid]

        # mask padding
        detection_saliency = detection_saliency * vid_mask.float()

        # gated residual: let the model learn how much refinement to apply
        memory_t = memory.transpose(0, 1)  # [B, L_vid, D]
        gate_values = self.gate(self.proj(memory_t)).squeeze(-1)  # [B, L_vid]
        gate_values = torch.tanh(gate_values)

        refined = saliency_scores + gate_values * detection_saliency
        return refined
