...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(  # noqa: WPS211
        ...
        gaussian_sigma: Optional[Tensor] = None,
    ):
        ...
        # Gaussian spatial bias on cross-attention
        ca_mask = src_mask
        if reference_points is not None and gaussian_sigma is not None:
            # reference_points: [Q, B, 2] (center, width), src: [L, B, D]
            centers = reference_points[:, :, 0]  # [Q, B]
            widths = reference_points[:, :, 1].clamp(min=1e-4)  # [Q, B]
            positions = torch.arange(length, device=src.device, dtype=src.dtype) / max(length - 1, 1)  # [L]
            # [Q, B, L]
            diff = positions[None, None, :] - centers[:, :, None]
            # gaussian_sigma can be scalar (dim=0) or per-query (Q, B)
            if gaussian_sigma.dim() == 2:
                # per-query sigma (LA Gaussian)
                sigma = (widths * gaussian_sigma).clamp(min=1e-4)[:, :, None]
            else:
                sigma = widths[:, :, None] * gaussian_sigma
            gauss_bias = -0.5 * (diff / sigma) ** 2  # [Q, B, L]
            # reshape to [B*nhead, Q, L]
            gauss_bias = gauss_bias.permute(1, 0, 2)  # [B, Q, L]
            gauss_bias = gauss_bias.unsqueeze(1).expand(-1, self.nhead, -1, -1)  # [B, H, Q, L]
            gauss_bias = gauss_bias.reshape(batch_size * self.nhead, num_queries, length)
            if ca_mask is None:
                ca_mask = gauss_bias
            else:
                ca_mask = ca_mask + gauss_bias
