...
class AdaptiveSoftLRMHC(nn.Module):
    ...
    def __init__(
        ...
        use_span_short_guard=False,
        span_short_guard_init=0.9,
        span_short_guard_boundary=10.0,
        span_short_guard_softness=2.0,
        use_span_query_gate=False,
        span_query_gate_hidden_dim=32,
        span_query_gate_init=0.1,
        isolate_base_grad=False,
        router_logit_limit=0.0,
        residual_limit=0.0,
    ):
        super().__init__()
        ...
        if use_span_short_guard and not 0.0 < span_short_guard_init < 1.0:
            raise ValueError("span_short_guard_init must be in (0, 1)")
        if span_short_guard_boundary <= 0.0:
            raise ValueError("span_short_guard_boundary must be positive")
        if span_short_guard_softness <= 0.0:
            raise ValueError("span_short_guard_softness must be positive")
        if use_span_query_gate and not 0.0 < span_query_gate_init < 1.0:
            raise ValueError("span_query_gate_init must be in (0, 1)")
        if router_logit_limit < 0.0:
            raise ValueError("router_logit_limit must be non-negative")
        ...
        if sum((uniform_routing, content_routing, hard_routing)) > 1:
            raise ValueError(
                "uniform_routing, content_routing and hard_routing are mutually exclusive"
            )
        ...
        self.use_span_short_guard = bool(use_span_short_guard)
        self.span_short_guard_softness = float(span_short_guard_softness)
        self.use_span_query_gate = bool(use_span_query_gate)
        self.isolate_base_grad = bool(isolate_base_grad)
        self.router_logit_limit = float(router_logit_limit)
        ...
        if use_span_refine:
            ...
            if self.use_span_short_guard:
                guard_logit = math.log(
                    span_short_guard_init / (1.0 - span_short_guard_init)
                )
                self.span_short_guard_logit = nn.Parameter(
                    torch.tensor(guard_logit, dtype=torch.float32)
                )
                self.span_short_guard_boundary_raw = nn.Parameter(
                    torch.log(torch.expm1(torch.tensor(span_short_guard_boundary)))
                )
            if self.use_span_query_gate:
                self.span_query_gate = nn.Sequential(
                    nn.Linear(hidden_dim + 8, span_query_gate_hidden_dim),
                    nn.GELU(),
                    nn.Linear(span_query_gate_hidden_dim, 1),
                )
                nn.init.xavier_uniform_(self.span_query_gate[0].weight)
                nn.init.zeros_(self.span_query_gate[0].bias)
                nn.init.zeros_(self.span_query_gate[2].weight)
                span_gate_logit = math.log(
                    span_query_gate_init / (1.0 - span_query_gate_init)
                )
                nn.init.constant_(self.span_query_gate[2].bias, span_gate_logit)
    ...
    def forward(
        ...
        router_width = torch.zeros_like(width_logit) if self.content_routing else width_logit
        router_input = torch.cat((query_features.detach(), router_width), dim=-1)
        router_logits = self.router(router_input)
        if self.router_logit_limit > 0.0:
            router_logits = self.router_logit_limit * torch.tanh(
                router_logits / self.router_logit_limit
            )
        route_logits = router_logits if self.content_routing else prior_logits + router_logits
        ...
        expert_features = (
            query_features.detach() if self.isolate_base_grad else query_features
        )
        expert_residuals = torch.cat(
            [expert(expert_features) for expert in self.experts], dim=-1
        )
        if self.residual_limit > 0.0:
            expert_residuals = self.residual_limit * torch.tanh(
                expert_residuals / self.residual_limit
            )
        routed_residual = (route_weights * expert_residuals).sum(dim=-1, keepdim=True)
        query_gate = torch.ones_like(routed_residual)
        if (
            self.use_query_gate
            and not self.uniform_routing
            and self.use_gate
        ):
        ...
        residual = self.output_scale * route_gate * routed_residual

        correction_base_logits = (
            base_logits.detach() if self.isolate_base_grad else base_logits
        )
        corrected_logits = correction_base_logits.clone()
        ...
        ungated_routed_spans = None
        span_query_gate = None
        span_short_guard = None
        if self.use_span_refine:
            ...
            expert_span_deltas = torch.stack(
                [expert(expert_features) for expert in self.span_experts], dim=-2
            )
            ...
            span_short_guard = torch.ones_like(route_gate)
            if self.use_span_short_guard:
                guard_strength = self.span_short_guard_logit.sigmoid()
                if predicted_duration is None:
                    short_responsibility = prior_weights[..., :1].sqrt()
                else:
                    duration = predicted_duration.detach().to(route_gate).unsqueeze(-1)
                    boundary = F.softplus(self.span_short_guard_boundary_raw).to(
                        route_gate
                    )
                    short_responsibility = torch.sigmoid(
                        (boundary - duration) / self.span_short_guard_softness
                    )
                span_short_guard = (
                    1.0 - guard_strength * short_responsibility
                ).clamp_min(0.0)
            ungated_span_delta = (
                self.span_output_scale
                * route_gate
                * span_short_guard
                * routed_span_delta
            )
            span_query_gate = torch.ones_like(route_gate)
            if self.use_span_query_gate:
                span_gate_input = torch.cat(
                    (
                        query_features.detach(),
                        width_logit,
                        route_weights.detach(),
                        routed_span_delta.detach(),
                        base_spans.detach(),
                    ),
                    dim=-1,
                )
                span_query_gate = self.span_query_gate(span_gate_input).sigmoid()
            applied_span_delta = span_query_gate * ungated_span_delta
            correction_base_spans = (
                base_spans.detach() if self.isolate_base_grad else base_spans
            )
            base_span_logits = torch.logit(
                correction_base_spans.clamp(1e-4, 1.0 - 1e-4)
            )
            ungated_routed_spans = (
                base_span_logits + ungated_span_delta
            ).sigmoid()
        ...
        if self.use_span_refine:
            diagnostics.update({
                ...
                "ungated_routed_spans": ungated_routed_spans,
                "span_query_gate": span_query_gate,
                "span_short_guard": span_short_guard,
                "span_short_guard_boundary": F.softplus(
                    self.span_short_guard_boundary_raw
                ).view(*([1] * route_weights.ndim)).expand(
                    *route_weights.shape[:-1], 1
                ) if self.use_span_short_guard else route_gate.new_zeros(route_gate.shape),
            })
