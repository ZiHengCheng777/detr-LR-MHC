...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    if opt.dset_name == "charadesSTA":
        post_processor = PostProcessorDETR(
            clip_length=opt.clip_length, min_ts_val=0, max_ts_val=150,
            min_w_l=1, max_w_l=150, move_window_method="left",
    ...
    if opt.dset_name in ['tvsum']:
        ...
        if opt.dset_name == "charadesSTA":
            for item in submission:
                item.pop("pred_saliency_scores", None)
...
def setup_model(opt):
    ...
    if getattr(opt, "lrmhc_train_span_only", False):
        if not getattr(opt, "use_lrmhc", False) or not getattr(opt, "lrmhc_span_refine", False):
            raise ValueError("--lrmhc_train_span_only requires --use_lrmhc and --lrmhc_span_refine")
        for name, parameter in model.named_parameters():
            parameter.requires_grad_(name.startswith("lrmhc.span_experts."))
        logger.info("LR-MHC span-only stage: only routed span experts are trainable")
    elif getattr(opt, "use_lrmhc", False) and opt.lrmhc_freeze_base:
        for name, parameter in model.named_parameters():
            if not name.startswith("lrmhc."):
                parameter.requires_grad_(False)
        logger.info("LR-MHC plug-in stage: base DETR parameters are frozen")
    ...
    for name, parameter in model.named_parameters():
        ...
        if name in {"lrmhc.center_start", "lrmhc.gap_raw"}:
            prototype_params.append(parameter)
        elif name.startswith("lrmhc."):
            lrmhc_params.append(parameter)
        else:
    ...
    param_dicts = []
    if base_params:
        param_dicts.append({"params": base_params, "lr": opt.lr})
    if lrmhc_params:
        param_dicts.append({"params": lrmhc_params, "lr": opt.lr * opt.lrmhc_lr_mult})
    if prototype_params:
        param_dicts.append({
            "params": prototype_params,
            "lr": opt.lr * opt.lrmhc_lr_mult * opt.lrmhc_prototype_lr_mult,
        })
    ...
    if opt.resume is not None:
        ...
        incompatible = model.load_state_dict(
            checkpoint["model"], strict=not getattr(opt, "use_lrmhc", False)
        )
        if getattr(opt, "use_lrmhc", False):
            logger.info("LR-MHC warm start missing keys: %s", incompatible.missing_keys)
        if opt.resume_all:
