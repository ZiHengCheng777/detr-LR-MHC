...
def dict_to_markdown(d, max_str_len=120):
    # Keep sequence-valued options in a single table cell.
    d = {k: repr(v) if isinstance(v, (list, tuple, dict, set, np.ndarray)) else v
         for k, v in d.items()}
    # truncate string that is longer than max_str_len
    if max_str_len is not None:
        d = {k: v[-max_str_len:] if isinstance(v, str) else v for k, v in d.items()}
    return pd.DataFrame(d.items(), columns=["option", "value"]).set_index("option").to_markdown()
