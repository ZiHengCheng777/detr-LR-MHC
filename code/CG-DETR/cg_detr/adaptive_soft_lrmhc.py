...
def quality_aware_routed_rank_loss(
    ...
    for sample_idx, target in enumerate(targets["span_labels"]):
        ...
        if balance_durations and video_durations is not None:
            ...
            # Queries matching any GT window must not become negatives for another
            # duration group in the same sample.
...
class AdaptiveSoftLRMHC(nn.Module):
    ...
    def __init__(
        ...
        # The expert outputs are residual corrections, so their global scales
        # may be signed without changing the router probabilities.
    ...
    def forward(self, query_features, base_logits, predicted_width, base_spans=None):
        ...
        if self.use_span_refine:
            ...
            applied_span_delta = (
                self.span_output_scale * route_gate * routed_span_delta
            )
