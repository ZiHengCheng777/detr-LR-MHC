...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,
        ...
        if self.dset_name == 'charadesSTA' and self.use_glove:
            if vocab is None:
                raise ImportError("torchtext is required for Charades-STA VGG features")
    ...
    def _get_query_feat_by_qid(self, qid):
        ...
        # youtube-hl
            ...
            if not exists(q_feat_path):
                q_feat_path = join(self.q_feat_dir, f"{qid}.npz")
            q_feat_file = np.load(q_feat_path)
            q_feat_key = self.q_feat_type if self.q_feat_type in q_feat_file else "features"
            q_feat = q_feat_file[q_feat_key].astype(np.float32)
            if q_feat.ndim == 3 and q_feat.shape[0] == 1:
                q_feat = q_feat[0]
            if self.q_feat_type == "last_hidden_state":
                q_feat = q_feat[:self.max_q_l]
            if self.normalize_t:
        ...
        if self.dset_name == 'tvsum':
            ...
            for _feat_dir in self.v_feat_dirs:
                try:
                    ...
                    if exists(_feat_path):
                        _feat = torch.load(
                            _feat_path, map_location="cpu"
                        )[:self.max_v_l].float().numpy()
                    else:
                        _feat_path = join(_feat_dir, f"{vid}.npy")
                        _feat = np.load(_feat_path)[:self.max_v_l].astype(np.float32)
                if self.normalize_v:
