...
def eval_moment_retrieval(submission, ground_truth, verbose=True,dataset='hl'):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        if not _ground_truth:
            empty_scores = {
                str(float(f"{threshold:.2f}")): 0.0
                for threshold in np.linspace(0.5, 0.95, 10)
            }
            ret_metrics[name] = {
                "MR-mAP": {**empty_scores, "average": 0.0},
                "MR-R1": empty_scores.copy(),
            }
            continue
...
def eval_submission_cha(submission, ground_truth, verbose=True, match_number=True, dataset='hl'):
    ...
    # Charades-STA has moment annotations only. Some converted records retain
    # placeholder saliency fields, which must not trigger QVHighlights scoring.
