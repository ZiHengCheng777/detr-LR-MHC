...
from uvcom.adaptive_soft_lrmhc import (
    AdaptiveSoftLRMHC,
...
class UVCOM(nn.Module):
    ...
    def __init__(self, CIM, position_embed, txt_position_embed, txt_dim, vid_dim,
                 ...
                 aud_dim=0, neg_choose_epoch=80, args=None):
        ...
        if self.use_lrmhc:
            ...
            with torch.random.fork_rng(devices=[]):
                self.lrmhc = AdaptiveSoftLRMHC(
                    ...
                    eval_output_scale=getattr(args, "lrmhc_eval_output_scale", None),
                    use_span_refine=getattr(args, "lrmhc_span_refine", False),
                    span_output_scale=getattr(args, "lrmhc_span_output_scale", 0.1),
                    eval_span_output_scale=getattr(
                        args, "lrmhc_eval_span_output_scale", None
                    ),
    ...
    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, epoch=None, src_aud=None, src_aud_mask=None):
        ...
        if self.use_lrmhc:
            ...
            outputs_class, outputs_coord, route_diagnostics = self.lrmhc(
                hs, outputs_class, final_width, outputs_coord
            )
...
class SetCriterion(nn.Module):
    ...
    def _duration_responsibilities(self, outputs, idx, tgt_spans):
        """Return smooth short/middle/long memberships in absolute seconds."""
    ...
    def loss_spans(self, outputs, targets, indices):
        ...
        if (
            ...
            corrected_cost = loss_span.mean(dim=-1) + loss_giou
        ...
        if (
            ...
            per_match_loss = loss_span.mean(dim=-1) + loss_giou
            expert_loss = (
