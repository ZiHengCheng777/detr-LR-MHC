...
def setup_model(opt):
    ...
    for name, parameter in model.named_parameters():
        if not parameter.requires_grad:
            continue
        (lrmhc_params if name.startswith("lrmhc.") else base_params).append(parameter)
    trainable_count = sum(
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=opt.lr_drop, gamma=opt.lr_gamma
    )
    ...
    if opt.resume is not None:
        ...
        if getattr(opt, "use_lrmhc", False):
            ...
            logger.info("LR-MHC warm start unexpected keys: %s", incompatible.unexpected_keys)
